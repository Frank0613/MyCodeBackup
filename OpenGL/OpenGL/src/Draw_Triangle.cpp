...
void FillTriangleArea() {
  ...
  // Detect every point in the bounding box
  for (int i = minX / EachGridLength; i <= maxX / EachGridLength; i++) {
      for (int j = minY / EachGridLength; j <= maxY / EachGridLength; j++) {
          ...
          if (IsPointInTriangle(x, y)) {
                ...
                grid[i + WholeGridScale][j + WholeGridScale].second = {1.0f, 1.0f, 0};
          }
      }
  }
}
    ...
    if (button == GLUT_LEFT_BUTTON) {
        ...
        if (state == GLUT_DOWN) {
            ...
            //Second click
                ...
                MidPointFunction(ClickPosInGrid_X3, ClickPosInGrid_Y3,
                                 ClickPosInGrid_X1, ClickPosInGrid_Y1);
                FillTriangleArea();
